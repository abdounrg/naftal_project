...
const TPEMaintenance = () => {
  ...
  const emptyForm = { serial: '', model: '', structure_code: '', station_code: '', operation_mode: '', breakdown_date: '', diagnostic: '', status: 'en_traitement' };
  ...
  const openEditModal = (row: any) => {
    ...
    setFormData({
      ...
      diagnostic: row.diagnostic || '', status: row.status || 'en_traitement',
    });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && selectedRow) { await tpeApi.updateMaintenance(selectedRow.id, formData); }
      else { await tpeApi.createMaintenance(formData); }
            ...
            <form onSubmit={handleSubmit} className="p-6">
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{language === 'fr' ? 'Date Panne' : 'Breakdown Date'}</label>
                    <input type="date" value={formData.breakdown_date} onChange={e => setFormData(p => ({ ...p, breakdown_date: e.target.value }))} className={inputClass} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{language === 'fr' ? 'Diagnostic' : 'Diagnostic'}</label>
                  ...
                  <div>
                    ...
                    <select required value={formData.status} onChange={e => setFormData(p => ({ ...p, status: e.target.value }))} className={inputClass}>
                      {['en_traitement', 'a_retourner', 'repare', 'irreparable', 'remplace', 'changement_sim', 'reconfigure', 'retourne'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
                    </select>
                  </div>
                </div>
              </div>
